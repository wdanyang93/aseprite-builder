...
## 1-2. 비율은 AI 에게 맡기지 않는다 ★

AI 는 같은 것을 다시 요구할수록 조금씩 작게 그리는 경향이 있다. 시트끼리 비율이 안 맞는 것이
이 프로젝트의 원래 고민이었는데, **맞추려 애쓸 필요가 없다.** 파츠마다 길이를 재서 리그의 뼈
길이에 맞춰 균일 확대·축소하면 된다.

```bash
python -m rigkit fit   assets/parts          # 각 파츠의 배율을 재서 parts.json 에 넣는다
python -m rigkit check assets/parts          # 시트 안에서 파츠끼리 비율이 맞는지 본다
```

- `fit` 은 이미지를 건드리지 않는다. 렌더·Godot 내보내기에서 쓰는 `scale` 값만 적는다.
- 0.31배로 그려 왔든 3.4배로 그려 왔든 뼈 길이에 2% 이내로 맞는다
  (`tests/test_fit.py::test_any_source_size_lands_on_the_rig`).
- `check` 는 **시트 안에서** 파츠끼리 어긋난 정도를 본다. 전체가 작게 그려진 건 문제가 아니고,
  부츠만 혼자 30% 작은 것이 문제다. ±8% 를 넘으면 그 파츠만 다시 요구한다.

즉 AI 에게 요구할 것은 "정확한 크기"가 아니라 **"한 시트 안에서는 서로 비율이 맞을 것"** 하나뿐이다.

## 1-3. 포즈 변형 시트는 필요 없다 ★

팔·다리를 여러 각도로 그린 포즈 시트(자연 하강 / 앞으로 / 뒤로 / 굽힘 …)는 리깅에서는 쓰지 않는다.
굽힘은 뼈대가 만든다. 그런 시트를 계속 요구하는 것이 비율이 무너지는 원인이기도 하다.

필요한 것은 **뷰마다 곧게 편 파츠 한 벌**이다. 통짜로 온 팔다리는 관절에서 나눈다:

```bash
python -m rigkit split assets/raw/leg_front.png thigh_l shin_l -o assets/parts --at 0.5 --overlap 12
```
`--at` 은 불투명 영역 높이에서 자를 위치(무릎은 0.5 근처), `--overlap` 은 회전했을 때 관절이
벌어지지 않도록 양쪽에 남기는 여유다. 화소는 잃지 않는다.
